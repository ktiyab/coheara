...
fn process_fresh_job(
    ...
    let mut processor = crate::pipeline::processor::build_processor_from_assignment_with_vision(
        &assignment,
        &pipeline_config,
        &lang,
        has_gpu,
...
/// C4-FIX: Detect GPU availability for strategy resolution.
fn detect_gpu(state: &CoreState) -> bool {
    state
        .butler()
        .hardware_tier()
        .map_or(false, |t| {
