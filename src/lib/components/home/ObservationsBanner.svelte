...
<section class="px-6 py-2" aria-label={$t('home.observations_heading') ?? 'Health observations'}>
  ...
  <div class="flex flex-col gap-2" role="list" aria-live="polite">
    {#each alerts as alert (alert.id)}
      <div
        class="bg-white border border-stone-200 rounded-xl px-4 py-3 flex items-start gap-3"
        role="listitem"
      >
        <span
          class="flex-shrink-0 w-6 h-6 rounded-full bg-stone-100 flex items-center justify-center text-xs font-bold {typeColor[alert.alert_type]}"
          aria-hidden="true"
        >
          {typeIcon[alert.alert_type]}
        </span>
        <div class="flex-1 min-w-0">
          <p class="text-sm text-stone-800">{alert.patient_message}</p>
          <p class="text-xs text-stone-500 mt-0.5">
            {new Date(alert.detected_at).toLocaleDateString()}
          </p>
        </div>
        <button
          class="flex-shrink-0 text-stone-500 hover:text-stone-600 min-h-[44px] min-w-[44px]
