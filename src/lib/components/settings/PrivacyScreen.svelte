...
<div class="flex flex-col min-h-screen pb-20 bg-stone-50">
  <header class="px-6 pt-6 pb-4">
    <h1 class="text-2xl font-bold text-stone-800">{$t('settings.privacy_heading')}</h1>
  </header>
  ...
  {:else if privacyInfo}
    <div class="px-6 space-y-4">
            ...
            <div class="flex justify-between">
              ...
              <span class="text-stone-800">
                ...
                {#if profile.aiStatus.active_model.quality === 'Medical'}
                  <span class="text-xs text-teal-600 ml-1">{$t('settings.ai_medical')}</span>
                {:else}
                  <span class="text-xs text-stone-400 ml-1">{$t('settings.ai_general')}</span>
                {/if}
              </span>
            </div>
          {/if}
      ...
      <section class="bg-blue-50 rounded-xl p-5 border border-blue-100">
        <h2 class="text-sm font-medium text-blue-700 mb-3">{$t('settings.verify_yourself')}</h2>
        <ol class="text-sm text-blue-800 space-y-1 list-decimal list-inside">
          <li>{$t('settings.verify_step1')}</li>
          <li>{$t('settings.verify_step2')}</li>
          <li>{$t('settings.verify_step3')}</li>
        </ol>
        <p class="text-xs text-blue-600 mt-2">{$t('settings.verify_explanation')}</p>
      </section>
